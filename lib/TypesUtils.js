...
var base_wallet = require('jcc_jingtum_base_lib').Wallet;
var KeyPair = require('jcc_jingtum_base_lib').KeyPair
 ...
 */
function convertUint160ToByteArray(in_str, token) {
    token = token || 'swt'
    var i, out = [],
        len;
    var str = in_str.replace(/\s|0x/g, "");

    if (base_wallet.isValidAddress(str, token)) {
        var inst = new KeyPair(token)
        out = inst.convertAddressToBytes(str);
    } else {
...
function typeTest(in_str, token) {
    token = token || 'swt'
    var b1 = UInt160.from_json(in_str).to_bytes();
    var b2 = convertUint160ToByteArray(in_str, token);
...
var STHash160 = exports.Hash160 = new SerializedType({
    ...
    parse: function (so) {
        var inst = new KeyPair(so._token);
        return inst.convertBytesToAddress(so.read(20));
    }
});
...
var STAmount = exports.Amount = new SerializedType({
    serialize: function (so, val) {
        var amount = Amount.from_json(val, so._token);
        ...
        if (amount.is_native()) {
            ...
            var inst = new KeyPair(so._token);
            so.append(inst.convertAddressToBytes(amount.issuer()));

        }

    },
    parse: function (so) {
        var amount = new Amount(so._token);
        ...
        if (value_bytes[0] & 0x80) {
            ...
            var inst = new KeyPair(so._token);
            var issuer = inst.convertBytesToAddress(issuer_bytes); //UInt160.from_bytes(issuer_bytes);
...
var STAccount = exports.Account = new SerializedType({
    serialize: function (so, val) {
        var inst = new KeyPair(so._token);
        var byte_data = inst.convertAddressToBytes(val);
    ...
    parse: function (so) {
        ...
        var inst = new KeyPair(so._token);
        var result = inst.convertBytesToAddress(so.read(len)); //UInt160.from_bytes(so.read(len));
        ...
        for (var i = 0, l = val.length; i < l; i++) {
            ...
            for (var j = 0, l2 = val[i].length; j < l2; j++) {
                ...
                if (entry.account) {
                    ...
                    so.append(inst.convertAddressToBytes(entry.account));
                }
                ...
                if (entry.issuer) {
                    ...
                    so.append(inst.convertAddressToBytes(entry.issuer));
                }
            }
        }
