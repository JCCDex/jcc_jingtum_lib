...
var KeyPairs = require('jcc_jingtum-base-lib/src/keypairs.js');
 ...
 */
function convertUint160ToByteArray(in_str, base_wallet_currency) {
    var i, out = [], len;
    var str = in_str.replace(/\s|0x/g, "");
    var kp = new KeyPairs(base_wallet_currency);
    if (base_wallet.isValidAddress(str, base_wallet_currency)) {
        out = kp.convertAddressToBytes(str);
    } else {
...
function typeTest(in_str, base_wallet_currency) {
    var b1 = UInt160.from_json(in_str).to_bytes();
    var b2 = convertUint160ToByteArray(in_str, base_wallet_currency);
...
var STInt8 = exports.Int8 = new SerializedType({
    init: function(base_wallet_currency) {
        this._base_wallet_currency = base_wallet_currency;
        this.kp = new KeyPairs(this._base_wallet_currency);
    },
...
var STHash160 = exports.Hash160 = new SerializedType({
    ...
    parse: function (so) {
        return this.kp.convertBytesToAddress(so.read(20));
    }
});
...
var STAmount = exports.Amount = new SerializedType({
    ...
    serialize: function (so, val) {
        ...
        if (amount.is_native()) {
            ...
            so.append(this.kp.convertAddressToBytes(amount.issuer()));
    ...
    parse: function (so) {
        ...
        if (value_bytes[0] & 0x80) {
            ...
            var issuer = this.kp.convertBytesToAddress(issuer_bytes);//UInt160.from_bytes(issuer_bytes);
...
var STAccount = exports.Account = new SerializedType({
    ...
    serialize: function (so, val) {
        var byte_data = this.kp.convertAddressToBytes(val);
    ...
    parse: function (so) {
        ...
        var result = this.kp.convertBytesToAddress(so.read(len));//UInt160.from_bytes(so.read(len));
        ...
        for (var i = 0, l = val.length; i < l; i++) {
            ...
            for (var j = 0, l2 = val[i].length; j < l2; j++) {
                ...
                if (entry.account) {
                    ...
                    so.append(this.kp.convertAddressToBytes(entry.account));
                }
                ...
                if (entry.issuer) {
                    ...
                    so.append(this.kp.convertAddressToBytes(entry.issuer));
                }
            }
        }
...
function parse(so,base_wallet_currency) {
    ...
    type.init(base_wallet_currency);
