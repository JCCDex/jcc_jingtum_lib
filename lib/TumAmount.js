...
function Amount(base_wallet_currency) {
    // Json format:
    //  integer : SWT
    ...
    if(typeof arguments[0] === 'undefined'){
        throw new Error('Amount init: Input  has invalid wallet Currency!');
    } else {
        this._base_wallet_currency = arguments[0];
    }  
};
...
Amount.prototype.parse_issuer = function (issuer) {
    if (base_wallet.isValidAddress(issuer,this._base_wallet_currency))
        this._issuer = issuer;
        ...
        if (!isTumCode(in_json.currency))
            ...
            if (in_json.currency != 'SWT') {
                ...
                if (typeof in_json.issuer != 'undefined' &&
                    in_json.issuer != null) {
                    if (base_wallet.isValidAddress(in_json.issuer, this._base_wallet_currency)) {
