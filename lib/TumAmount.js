...
Amount.from_json = function (j, token) {
    token = token || 'swt'
    return (new Amount(token)).parse_json(j);
};
...
Amount.prototype.parse_issuer = function (issuer) {
    if (base_wallet.isValidAddress(issuer, this._token))
        this._issuer = issuer;
        ...
        if (!isTumCode(in_json.currency))
            ...
            if (in_json.currency != 'SWT') {
                ...
                if (typeof in_json.issuer != 'undefined' &&
                    in_json.issuer != null) {
                    if (base_wallet.isValidAddress(in_json.issuer, this._token)) {
