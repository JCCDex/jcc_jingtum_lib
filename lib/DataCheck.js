...
function isRelation(in_str) {
    if (typeof (in_code) === 'string' && (in_str === 'freeze' || in_str === 'autorize')) {
        return true;
    } else {
        return false;
    }
...
function isAmount(in_obj, token) {
    ...
    if (typeof (in_obj.issuer) !== 'undefined' &&
        in_obj.issuer !== '') {
        if (!Wallet.isValidAddress(in_obj.issuer, token)) {
            return false;
        }
    } else {
        // if currency === 'SWT',自动补全issuer.
        if (in_obj.currency === 'SWT') {
            in_obj.issuer = '';
        } else {
            return false;
        }
...
function isBalance(in_obj, token) {
    ...
    if (!Wallet.isValidAddress(in_obj.counterparty, token)) {
        return false;
    }
