...
function isLetterNumer(in_str) {
    var numbers = /^[0-9A-Z]+$/;
    return !!String(in_str).match(numbers);
}

// return true if the code is 3 letters/numbers
function isCurrency(in_code) {
    return typeof (in_code) === 'string' ? (!!(in_code && in_code.length >= CURRENCY_NAME_LEN && in_code.length <= CURRENCY_NAME_LEN2)) : false;
}
 ...
 * currency
 */
function isAmount(in_obj, token) {
    ...
    if (typeof (in_obj.issuer) !== 'undefined' &&
        in_obj.issuer != '') {
        if (!base.isValidAddress(in_obj.issuer, token)) {
            return false;
        }
    } else {
 ...
 */
function buildAmount(in_currency, in_issuer, in_value, token) {
    if (!isTumCode(in_currency)) {
        throw new ParamException("Invalid currency code!");
    }

    if (!base.isValidAddress(in_issuer, token)) {
 ...
 */
function isBalance(in_obj, token) {
    if (typeof (in_obj) !== 'object') {
        return false;
    }
    ...
    if (!base.isValidAddress(in_obj.counterparty, token)) {
        return false;
    }
