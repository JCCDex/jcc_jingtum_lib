...
describe('test TypesUtils', function () {
    ...
    describe('test get_transaction_type api', function () {
        ...
        it('throw error if the structure is number but invalid', function () {
            expect(() => tu.get_transaction_type(102)).to.throw('Invalid transaction type!');
        })
        ...
        it('throw error if the structure is string but invalid', function () {
            expect(() => tu.get_transaction_type('SetFees')).to.throw('Invalid transaction type!');
        })

        it('throw error if the structure is not string and number', function () {
            expect(() => tu.get_transaction_type(null)).to.throw('Invalid input type for transaction type!');
        })
    })
    ...
    describe('test get_transaction_result api', function () {
        ...
        it('throw error if the structure is number but invalid', function () {
            expect(() => tu.get_transaction_result(142)).to.throw('Invalid transaction result!');
        })
        ...
        it('throw error if the structure is string but invalid', function () {
            expect(() => tu.get_transaction_result('aaaa')).to.throw('Invalid transaction result!');
        })

        it('throw error if the structure is not string and number', function () {
            expect(() => tu.get_transaction_result(null)).to.throw('Invalid input type for transaction result!');
        })
    })
    ...
    describe('test get_ledger_entry_type api', function () {
        ...
        it('throw error if the structure is number but invalid', function () {
            expect(() => tu.get_ledger_entry_type(142)).to.throw('Invalid input type for ransaction result!');
        })
    ...
    describe('test STArray', function () {
        ...
        it('throw error if keys\'s length is not equal to 1 when serialize', function () {
            ...
            expect(() => tu.Array.serialize(so, data)).to.throw('Cannot serialize an array containing non-single-key objects');
        })
    ...
    describe('test STObject', function () {
        ...
        it('success if serialize when no_marker is true', function () {
            ...
            expect(so.buffer).to.deep.equal([34, 0, 0, 0, 0, 104, 64, 0, 0, 0, 0, 0, 39, 16])
        ...
        it('success if serialize when no_marker is false', function () {
            ...
            expect(so.buffer).to.deep.equal([34, 0, 0, 0, 0, 104, 64, 0, 0, 0, 0, 0, 39, 16, 225])
            expect(so.pointer).to.equal(so.buffer.length)
        })
    })
    ...
    describe('test STHash128', function () {
        ...
        it('throw error if the hash is invalid', function () {
            let so = new Serializer([])
            expect(() => tu.Hash128.serialize(so, '111111111111111111111111111111')).to.throw()
        })
    })
    ...
    describe('test STInt64', function () {
        ...
        it('throw error if val is less than 0', function () {
            let so = new Serializer([])
            expect(() => tu.Int64.serialize(so, -1)).to.throw('Negative value for unsigned Int64 is invalid.')
        })

        it('throw error if val is not valid hex int64', function () {
            let so = new Serializer([])
            expect(() => tu.Int64.serialize(so, 'aaaaaaaaaaaaaaaaaa')).to.throw('Not a valid hex Int64.')
        })

        it('throw error if val is not number and string', function () {
            let so = new Serializer([])
            expect(() => tu.Int64.serialize(so, null)).to.throw('Invalid type for Int64')
        })
